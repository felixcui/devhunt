          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative flex-shrink-0">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-blue-500 to-green-500 rounded-lg sm:rounded-xl flex items-center justify-center shadow-soft">
                  <FiBookOpen className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                </div>
              </div>
              <div className="min-w-0">
                <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent line-clamp-1">
            ...
            <div className="flex items-center gap-2 sm:gap-3 self-start sm:self-center">
              ...
              <button
                ...
                disabled={loading}
                className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 bg-gradient-to-r from-blue-500 to-green-500 text-white rounded-lg hover:from-blue-600 hover:to-green-600 disabled:opacity-50 transition-all duration-200 hover:scale-105 shadow-soft text-xs sm:text-sm whitespace-nowrap"
              >
      ...
      {loading ? (
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="text-center py-8 sm:py-12">
            ...
            <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-1 sm:mb-2 px-4">正在加载相关资讯</h3>
            ...
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-soft animate-pulse">
                <div className="flex items-start gap-3 sm:gap-4">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-200 rounded-lg shimmer flex-shrink-0"></div>
        ...
        <div className="space-y-4 sm:space-y-6">
          {news.length === 0 ? (
            <div className="text-center py-12 sm:py-16 px-4">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-gray-600 mb-2">暂无相关资讯</h3>
              <p className="text-sm sm:text-base text-gray-500">暂未找到与此工具相关的资讯内容</p>
            </div>
          ) : (
            <div className="space-y-3 sm:space-y-4">
              {news.map((item, index) => (
                <article 
                  key={item.id} 
                  className="group relative bg-white rounded-xl sm:rounded-2xl shadow-soft hover:shadow-soft-lg p-4 sm:p-6 transition-all duration-300 hover:-translate-y-1 animate-slide-up border border-gray-100 hover:border-blue-200"
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className="flex items-start gap-3 sm:gap-4">
                    {/* 资讯图标 */}
                    <div className="flex-shrink-0 relative">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-blue-500 to-green-500 rounded-lg sm:rounded-xl flex items-center justify-center shadow-soft group-hover:scale-110 transition-transform duration-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4 mb-2 sm:mb-3">
                        ...
                        >
                          <h2 className="text-base sm:text-lg font-semibold text-gray-900 group-hover/link:text-blue-600 transition-colors duration-200 leading-tight mb-1 sm:mb-2 flex items-start gap-1.5 sm:gap-2">
                            <span className="flex-1">{item.title}</span>
                            <FiExternalLink className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 group-hover/link:text-blue-600 transition-colors duration-200 flex-shrink-0 mt-0.5 sm:mt-1" />
                          </h2>
                        </a>
                      ...
                      {item.tool && (
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm text-blue-600 bg-blue-50 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border border-blue-200 hover:bg-blue-100 transition-colors">
                  ...
                  {/* 悬浮时的底部装饰 */}
                  <div className="absolute bottom-0 left-4 right-4 sm:left-6 sm:right-6 h-0.5 sm:h-1 bg-gradient-to-r from-blue-500 to-green-500 rounded-full transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></div>
                </article>
              ))}
            </div>
          )}
